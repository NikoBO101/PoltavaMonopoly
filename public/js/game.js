...
let currentRound = 1; let debtAlertShown = false; let jackpotRate = 0.5;
...
let myMultiplayerId = null; let currentLobby = null; let pendingTrade = null;
...
const playerColors = ['#ef4444', '#3b82f6', '#f59e0b', '#8b5cf6', '#10b981', '#f97316'];
...
if(socket) {
    ...
    socket.on('updateRoomsList', (roomsList) => {
        ...
        if(roomsList.length === 0) { container.innerHTML = '<div style="color:#94a3b8; text-align:center; padding:10px;">Немає відкритих кімнат.</div>'; return; }
        let html = '';
        roomsList.forEach(r => {
            let lock = r.hasPassword ? '🔒' : '🔓'; let status = r.status === 'waiting' ? '<span style="color:#10b981;">Очікування</span>' : '<span style="color:#ef4444;">В грі</span>';
            html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:10px; background:rgba(0,0,0,0.3); border-radius:8px; margin-bottom:5px;"><div><b>${r.name}</b> <span style="font-size:10px; color:#94a3b8;">(Код: ${r.id})</span><br><span style="font-size:11px;">${lock} Гравців: ${r.playersCount}/6 | ${status}</span></div><button class="btn-green" style="width:auto; margin:0;" onclick="joinRoomFromList('${r.id}', ${r.hasPassword})">Увійти</button></div>`;
        });
    ...
    socket.on('diceRolled', async (data) => {
        if (isRolling || processDebts()) return; isRolling = true; updateUI();
        playSound('sfx-dice'); const d1 = document.getElementById('die1'), d2 = document.getElementById('die2');
    ...
    socket.on('updateGameState', (stateData) => {
        ...
        jackpotAmount = stateData.jackpotAmount; stocks = stateData.stocks; currentRound = stateData.currentRound || currentRound;
...
function broadcastState() { if (isOnlineMode && isMyTurn() && currentLobby) { socket.emit('syncGameState', currentLobby.id, { players, properties, turn, jackpotAmount, stocks, currentRound }); } }
function isMyTurn() { if (!isOnlineMode) return true; let activeP = players.find(x => x.debtMode) || players[turn]; return activeP && activeP.id === myMultiplayerId; }
...
function generatePlayerInputs() {
    ...
    for(let i=0; i<c; i++) {
        cont.innerHTML += `<div style="display:flex; align-items:center; gap:8px;"><input type="text" id="p${i}-name" value="${defs[i]}" style="flex-grow:1;"><label style="font-size:12px; font-weight:bold; color:#10b981; display:flex; align-items:center; gap:3px; background:rgba(0,0,0,0.5); padding:8px; border-radius:5px; cursor:pointer;"><input type="checkbox" id="p${i}-isbot" ${defs[i].includes('Бот') ? 'checked' : ''}> Бот</label></div>`;
    }
}
...
function changeRadio() { let val = document.getElementById('setting-radio').value; let bgm = document.getElementById('bgm'); if(!val) { bgm.pause(); } else { bgm.src = val; bgm.play().catch(e => {}); } }
...
function startLocalGame() {
  isOnlineMode = false; changeRadio(); currentRound = 1; jackpotAmount = 0;
  ...
  for(let i=0; i<c; i++) {
      ...
      players.push({ id: i, name: document.getElementById(`p${i}-name`).value, isBot: isBotChecked, color: playerColors[i], money: sm, deposit: 0, loan: 0, loanTurns: 0, pos: 0, inJail: false, jailTurns: 0, doublesCount: 0, isBankrupt: false, skipTurns: 0, reverseMove: false, portfolio: { PTC: 0, RTL: 0, TRN: 0, PST: 0, GOV: 0 }, stockHistory: [], debtMode: false });
  }
...
function renderLobbyPlayers(playersList) {
    ...
    playersList.forEach((p, i) => { let hostBadge = p.isHost ? '👑 ' : ''; let youBadge = p.id === myMultiplayerId ? ' <span style="color:#94a3b8; font-size:12px;">(Ви)</span>' : ''; if(p.id === myMultiplayerId && p.isHost) iAmHost = true; html += `<div style="background:#1e293b; padding:10px; border-radius:8px; margin-bottom:5px; border-left:4px solid ${playerColors[i % playerColors.length]}; font-weight:bold;">${hostBadge}${p.name}${youBadge}</div>`; });
...
function updatePropertyColors() {
    for(let i=0; i<40; i++) {
        ...
        if(properties[i] && !properties[i].isMortgaged) { let pColor = players.find(x => x.id === properties[i].owner).color; ownerBar.style.backgroundColor = pColor; cell.style.borderColor = pColor; cell.style.boxShadow = `inset 0 0 15px ${pColor}44`; } 
        else { ownerBar.style.backgroundColor = 'transparent'; cell.style.borderColor = '#cbd5e1'; cell.style.boxShadow = 'none';}
    }
}
...
function updateUI() {
  ...
  players.forEach(p => {
    ...
    dash.innerHTML += `<div class="player-stat ${activeClass} ${p.isBankrupt ? 'bankrupt-stat' : ''}"><div><div class="color-dot" style="background:${p.color}"></div>${p.name} ${p.isBot?'🤖':''} ${iconHTML}</div><span style="color: ${p.money < 0 ? '#ef4444' : '#10b981'}; font-size:14px;">i₴${p.money}</span>${depHTML} ${cryptoHTML}</div>`;
  });
...
function logMsgLocal(msg) { const log = document.getElementById('log'); log.innerHTML = `<div style="margin-bottom:6px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:4px;">${msg}</div>` + log.innerHTML; }
function logMsg(msg) { logMsgLocal(msg); broadcastState(); }

function openSettingsModal() { switchTab('tab-settings'); document.getElementById('main-menu').style.display = 'flex'; }
...
function showInventory() {
    ...
    for(let i in properties) {
        if(properties[i].owner === p.id) {
            count++; let c = mapData[i]; let h = properties[i].houses; let status = properties[i].isMortgaged ? '<span style="color:#ef4444;">[ЗАСТАВА]</span>' : (h>0 ? `[Будинків: ${h}]` : '[Чиста]');
            html += `<div style="margin-bottom: 5px; padding: 8px; border: 1px solid #334155; border-radius: 6px; background: rgba(0,0,0,0.3); display: flex; align-items: center;"><div class="color-dot" style="background:${colors[c.group] || '#fff'}; margin-right: 10px;"></div><div style="flex-grow:1; font-size:14px;"><b>${c.name.replace('<br>',' ')}</b></div><div style="font-size: 11px; color: #94a3b8;">${status}</div></div>`;
        }
    }
    if(count === 0) html += `<div style="text-align:center; color:#94a3b8; padding: 20px;">У тебе ще немає майна.</div>`; html += `</div>`;
    openModal(`🎒 Інвентар: ${p.name}`, html, `<button class="btn-blue" onclick="closeModal()">Закрити</button>`);
}

function openCryptoMenu() {
  let p = players.find(x => x.debtMode) || players[turn]; p.stockHistory = p.stockHistory.filter(h => h.round > currentRound - 5);
  let boughtLast5 = p.stockHistory.reduce((s, h) => s + h.amount, 0); let availableToBuy = 5 - boughtLast5;
  ...
  ['PTC', 'RTL', 'TRN', 'PST', 'GOV'].forEach(sym => {
      ...
      let poolHtml = sym !== 'PTC' ? `<br><span style="font-size:11px; color:#f59e0b;">Пул дивідендів: i₴${s.pool}</span>` : ''; let availHtml = sym === 'GOV' ? `<br><span style="font-size:11px; color:#cbd5e1;">Доступно: ${s.totalMax - s.issued} шт.</span>` : '';
      html += `<div style="background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.1); padding: 10px; border-radius: 8px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; text-align: left;"><div><b>${sym}</b><br><span style="font-size:16px; font-weight:bold;">${arrow} i₴${s.price}</span><br><span style="font-size:12px;">В тебе: <b style="color:#06b6d4;">${p.portfolio[sym]} шт.</b></span>${poolHtml} ${availHtml}</div><div style="display:flex; flex-direction:column; gap:5px; width:45%;"><button class="btn-green" style="margin:0; padding:6px; font-size:12px;" onclick="tradeStock('${sym}', 1, true)" ${availableToBuy <= 0 || p.debtMode ? 'disabled' : ''}>Купити</button><button class="btn-red" style="margin:0; padding:6px; font-size:12px;" onclick="tradeStock('${sym}', 1, false)" ${p.portfolio[sym] <= 0 ? 'disabled' : ''}>Продати</button></div></div>`;
  });
...
function tradeStock(sym, amt, isBuying) {
  ...
  if(isBuying) {
      if(p.debtMode) return alert("Спочатку погаси борги!"); p.stockHistory = p.stockHistory.filter(h => h.round > currentRound - 5);
      ...
      let fee = Math.ceil(cost * 0.05); let totalCost = cost + fee;
      if(p.money < totalCost) return alert(`Немає грошей! З комісією: i₴${totalCost}`);
      p.money -= totalCost; playSound('sfx-spend'); p.portfolio[sym] += amt; p.stockHistory.push({round: currentRound, amount: amt});
      if(sym === 'GOV') s.issued += amt; logMsg(`📈 <b>${p.name}</b> купив акції ${sym}.`);
  } else {
      if(p.portfolio[sym] < amt) return alert("У тебе немає цих акцій!");
      p.portfolio[sym] -= amt; p.money += cost; playSound('sfx-earn');
      if(sym === 'GOV') s.issued -= amt; logMsg(`📉 <b>${p.name}</b> продав акції ${sym} за i₴${cost}.`);
  }
  updateUI(); checkDebtResolution(); openCryptoMenu(); broadcastState();
}

function distributeDividends(sym) {
  let s = stocks[sym]; if(s.pool < 100) return;
      ...
      if(divPerShare > 0) {
          ...
          players.forEach(pl => {
              if(!pl.isBankrupt && pl.portfolio[sym] > 0) {
                  let payout = divPerShare * pl.portfolio[sym]; pl.money += payout; logMsg(`📈 <b>${pl.name}</b> отримав дивіденди ${sym}: <b>+i₴${payout}</b>`);
...
function openDepositMenu() {
  ...
  let html = `<p style="font-size:16px;">Готівка: <b style="color:#10b981;">i₴${p.money}</b><br>У Банці: <b style="color:#f59e0b;">i₴${p.deposit}</b></p><p style="font-size:12px; color:#94a3b8;">Банка росте на 5% за кожен пройдений СТАРТ!</p><input type="number" id="deposit-amount" value="1000" min="100" step="100" style="width:100%; padding:12px; margin-bottom:15px; background:#0f172a; color:#fff; border:1px solid #475569; border-radius:8px; font-weight:bold; font-size:16px;">`;
  openModal("🏦 Моя Банка", html, `<button class="btn-green" onclick="makeDeposit(true)" ${p.debtMode?'disabled':''}>Покласти</button><button class="btn-gold" onclick="makeDeposit(false)">Зняти</button><button class="btn-blue" onclick="closeModal()">Закрити</button>`);
}
...
function makeDeposit(isAdding) {
  ...
  if(isAdding) {
      if(p.debtMode) return alert("Закрий борг перед поповненням депозиту!"); if(p.money < amt) return alert("Немає готівки!");
      p.money -= amt; p.deposit += amt; playSound('sfx-spend'); logMsg(`🏦 <b>${p.name}</b> поклав i₴${amt} в Банку.`);
  } else {
      if(p.deposit < amt) return alert("В Банці недостатньо коштів!");
      p.deposit -= amt; p.money += amt; playSound('sfx-earn'); logMsg(`🏦 <b>${p.name}</b> зняв i₴${amt} з Банки.`);
  }
  updateUI(); checkDebtResolution(); closeModal(); broadcastState();
}

function openLoanMenu() {
  let p = players.find(x => x.debtMode) || players[turn];
  if(p.loan === 0) { openModal("💳 Кредит", `<p style="font-size:15px;">Банк дає тобі <b style="color:#10b981;">i₴2000</b> прямо зараз.</p><p style="color:#ef4444; font-weight:bold; font-size:15px;">Але ти мусиш повернути i₴2500 протягом 5 ходів!</p>`, `<button class="btn-green" onclick="takeLoan()">Беру Кредит</button><button class="btn-blue" onclick="closeModal()">Відміна</button>`); } 
  else { openModal("💳 Погашення", `<p style="font-size:16px;">Твій борг: <b style="color:#ef4444;">i₴2500</b></p><p style="font-size:14px;">Залишилось ходів: <b>${p.loanTurns}</b></p>`, `<button class="btn-red" onclick="repayLoan()">Погасити борг</button><button class="btn-blue" onclick="closeModal()">Закрити</button>`); }
}
function takeLoan() { let p = players.find(x => x.debtMode) || players[turn]; p.money += 2000; p.loan = 2500; p.loanTurns = 5; playSound('sfx-earn'); logMsg(`💳 <b>${p.name}</b> взяв кредит i₴2000.`); updateUI(); checkDebtResolution(); closeModal(); broadcastState(); }
function repayLoan() { let p = players.find(x => x.debtMode) || players[turn]; if(p.money < p.loan) return alert("Не вистачає грошей для погашення!"); p.money -= p.loan; p.loan = 0; p.loanTurns = 0; playSound('sfx-spend'); logMsg(`💳 <b>${p.name}</b> успішно погасив борг.`); updateUI(); checkDebtResolution(); closeModal(); broadcastState(); }
...
function openTradeMenu() {
  ...
  let html = `<p>Обери гравця для угоди:</p><select id="trade-target" onchange="renderTradeLists()" style="width:100%; padding:12px; margin-bottom:15px; font-weight:bold; background:#0f172a; color:#fff; border:1px solid #475569; border-radius:8px;">`; activeOthers.forEach(op => html += `<option value="${op.id}">${op.name}</option>`); html += `</select><div id="trade-ui-container"></div>`;
...
function renderTradeLists() {
  ...
  const getList = (pid, prefix) => { let list = ''; for(let i in properties) { if(properties[i].owner === pid) { let c = mapData[i]; list += `<div style="display:flex; align-items:center; gap:5px; margin-bottom:6px; background:rgba(255,255,255,0.05); padding:6px; border-radius:6px; border:1px solid rgba(255,255,255,0.1);"><input type="checkbox" id="${prefix}-prop-${i}" value="${i}"> <div class="color-dot" style="background:${colors[c.group]}; width:14px; height:14px;"></div> <span style="font-size:12px; font-weight:bold;">${c.name.replace('<br>',' ')}</span></div>`; } } return list === '' ? '<div style="color:#94a3b8; font-size:12px; padding:10px 0;">Немає нерухомості</div>' : list; };
  document.getElementById('trade-ui-container').innerHTML = `<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; text-align:left;"><div style="background:var(--glass); padding:10px; border-radius:10px; border:1px solid #334155;"><h4 style="color:${p1.color}; margin-top:0; font-size:14px;">${p1.name} віддає:</h4><div style="font-size:12px; margin-bottom:5px;">Доплата (i₴):</div><input type="number" id="trade-money-give" value="0" min="0" max="${p1.money}" style="width:100%; padding:8px; margin-bottom:10px; background:#0f172a; color:#fff; border:1px solid #475569; border-radius:5px;"><div style="max-height:140px; overflow-y:auto; padding-right:5px;">${getList(p1.id, 'give')}</div></div><div style="background:var(--glass); padding:10px; border-radius:10px; border:1px solid #334155;"><h4 style="color:${p2.color}; margin-top:0; font-size:14px;">${p2.name} віддає:</h4><div style="font-size:12px; margin-bottom:5px;">Доплата (i₴):</div><input type="number" id="trade-money-take" value="0" min="0" max="${p2.money}" style="width:100%; padding:8px; margin-bottom:10px; background:#0f172a; color:#fff; border:1px solid #475569; border-radius:5px;"><div style="max-height:140px; overflow-y:auto; padding-right:5px;">${getList(p2.id, 'take')}</div></div></div>`;
}
function submitTrade() {
  ...
  if(mGive > p1.money) return alert("Немає стільки грошей для доплати!"); if(mTake > p2.money) return alert("У нього немає стільки грошей!");
  ...
  for(let i of [...pGive, ...pTake]) { for(let j in properties) { if(mapData[j].group === mapData[i].group && properties[j].houses > 0) return alert(`Спершу продайте всі будинки в цьому районі!`); } }
  ...
  if (!isOnlineMode && p2.isBot) {
      ...
      if (valGive >= valTake + (p2.name.includes('Важк') ? 500 : 100)) { logMsg(`🤖 <b>${p2.name}</b> погоджується на вигідну угоду!`); acceptTrade(); } 
  ...
  openModal(`Підтвердження Трейду`, `<p style="color:${p2.color}; font-size:18px; font-weight:bold; margin-bottom:5px;">Гей, ${p2.name}!</p><p style="margin-top:0;">${p1.name} пропонує обмін.</p><div style="display:flex; justify-content:space-between; text-align:left; background:var(--glass); padding:15px; border-radius:10px; border:1px solid rgba(255,255,255,0.1);"><div style="width:48%;"><b>Отримуєш:</b><br><br>${sGive}</div><div style="width:1px; background:rgba(255,255,255,0.2);"></div><div style="width:48%; padding-left:10px;"><b>Віддаєш:</b><br><br>${sTake}</div></div>`, `<button class="btn-green" onclick="acceptTrade()">Прийняти Угоду</button><button class="btn-red" onclick="closeModal()">Відмовитись</button>`);
}
...
async function startTurnLocal() {
  ...
  try {
      ...
      let v1 = Math.floor(Math.random()*6)+1; let v2 = Math.floor(Math.random()*6)+1; const isDouble = (v1 === v2);
      
      playSound('sfx-dice'); const d1 = document.getElementById('die1'), d2 = document.getElementById('die2');
      d1.classList.add('rolling-anim'); d2.classList.add('rolling-anim');
      for(let i=0; i<10; i++) { render2DDie('die1', Math.floor(Math.random()*6)+1); render2DDie('die2', Math.floor(Math.random()*6)+1); await sleep(50); }
      d1.classList.remove('rolling-anim'); d2.classList.remove('rolling-anim');
      render2DDie('die1', v1); render2DDie('die2', v2); lastDiceSum = v1 + v2; await sleep(300);
...
function handleLanding(index, p) {
  ...
  if(index === 20) { 
      if(jackpotAmount > 0) { 
          ...
          if (!p.isBot) { openModal(`🎉 ДЖЕКПОТ ПАРКОВКИ!`, `<p style="font-size:24px; color:#10b981; font-weight:bold; margin:10px 0;">+i₴${j}</p>`, `<button class="btn-green" onclick="closeModal(); nextTurn();">Забрати гроші</button>`); } else { setTimeout(() => nextTurn(), 1500); }
          return;
      } 
      return nextTurn(); 
  }
  if(cell.type === 'tax') { 
      ...
      else { openModal(`Податок`, `<p style="font-size:16px;">Державний платіж. До сплати: <b style="color:#ef4444;">i₴${cell.amount}</b></p>`, `<button class="btn-red" onclick="payTax(${cell.amount})">Заплатити</button>`); }
      return; 
  }
  ...
  if(cell.type === 'chance' || cell.type === 'news') { 
      ...
      else { openModal(isUrgent ? "⚡ БЛИСКАВКА" : "Картка", `<p style="font-size:16px; ${isUrgent?'color:#ef4444; font-weight:bold;':''}">${window.currentCard.text}</p>`, `<button class="btn-blue" onclick="applyCard();">Ок</button>`); }
      return; 
  }
  ...
  if(cell.price) {
      ...
      if(!prop) { 
          if (p.isBot) {
              ...
              if (p.money >= cell.price + buffer) { setTimeout(() => buyProperty(index), 1500); } else { setTimeout(() => skipProperty(), 1000); }
          }
          else { openModal(`Купівля`, `<p style="font-size:16px; margin-bottom:5px;">Купити <b>${cell.name.replace('<br>',' ')}</b>?</p><p style="margin-top:0;">Ціна: <b style="color:#10b981; font-size:18px;">i₴${cell.price}</b></p>`, `<button class="btn-green" onclick="buyProperty(${index})" ${p.money<cell.price?'disabled':''}>Купити</button><button class="btn-red" onclick="skipProperty()">Відмовитись</button>`); }
      } 
...
function payRent(index, p, propData) {
  ...
  else { openModal(`Оренда`, `<p style="font-size:16px;">Власник: <b>${owner.name}</b><br>До сплати: <b style="color:#ef4444; font-size:18px;">i₴${rent}</b></p>`, `<button class="btn-red" onclick="payRentConfirm(${index}, ${owner.id}, ${rent})">Заплатити</button>`); }
}
...
function showPropertyInfo(index) {
  ...
  let rentDetails = '';
  if(cell.type === 'station') { rentDetails = `Оренда залежить від кількості АЗС.<br>Базова: i₴${cell.baseRent} (х2, х4, х8)`; }
  else if(cell.type === 'utility') { rentDetails = `Оренда: Сума кубиків × 100 (або × 250)`; }
  else { const rentArr = getRentArray(cell.baseRent); rentDetails = `Оренда: <b style="color:#10b981;">i₴${rentArr[0]}</b><br><div class="prop-card-row"><span>З 1 Будинком</span><span>i₴${rentArr[1]}</span></div><div class="prop-card-row"><span>З 2 Будинками</span><span>i₴${rentArr[2]}</span></div><div class="prop-card-row"><span>З 3 Будинками</span><span>i₴${rentArr[3]}</span></div><div class="prop-card-row"><span>З 4 Будинками</span><span>i₴${rentArr[4]}</span></div><div class="prop-card-row"><span style="color:#ef4444;font-weight:bold;">З ГОТЕЛЕМ</span><span style="color:#ef4444;font-weight:bold;">i₴${rentArr[5]}</span></div>`; }

  let html = `<div class="prop-card"><div class="prop-card-header" style="background-color: ${colors[cell.group]}; color: ${cell.group==='yellow'||cell.group==='none'?'#000':'#fff'}">${cell.name.replace('<br>',' ')}</div><div class="prop-card-body">${rentDetails}</div><div class="prop-card-footer"><div>Вартість будинку: <b style="color:#10b981;">i₴${cell.housePrice || '-'}</b></div><div>Сума застави: <b style="color:#f59e0b;">i₴${mortgageValue}</b></div><div style="margin-top:8px; font-weight:bold; font-size:14px;">Власник: ${ownerName} ${prop && prop.isMortgaged ? '<span style="color:#ef4444;">(ЗАСТАВА)</span>' : ''}</div></div></div>`;
...
function unmortgage(i, a) { let p = players.find(x => x.debtMode) || players[turn]; if(p.money<a) return alert("Немає грошей!"); p.money-=a; properties[i].isMortgaged=false; playSound('sfx-spend'); updateUI(); checkDebtResolution(); closeModal(); broadcastState(); }
function buildHouse(index) {
  ...
  if(prop.houses >= 5) return alert("Тут вже стоїть готель!"); if(prop.houses > minHouses) return alert("Будуй рівномірно!");
...
function payTax(amount) { deductMoney(players[turn], amount); logMsg(`Сплачено податок: i₴${amount}`); closeModal(); if(!processDebts()) nextTurn(); }
...
function applyCard() {
  ...
  if(c.action === 'pay') { deductMoney(p, c.val); }
      ...
      let totalEffect = 0; players.forEach(pl => { if(pl.isBankrupt) return; let count = 0; for(let i in properties) { if(mapData[i].group === c.group && properties[i].owner === pl.id) count++; }
      ...
      let totalPaid = 0; players.forEach(pl => { if(pl.isBankrupt) return; if(!owners[pl.id]) { deductMoney(pl, c.val); totalPaid += c.val; } });
      ...
      if(activeOwners.length > 0 && totalPaid > 0) {
          ...
          activeOwners.forEach(oid => { let share = Math.floor(totalPaid * (owners[oid] / totalProps)); players.find(x=>x.id==oid).money += share; logMsg(`💼 Власник отримує i₴${share} прибутку.`); }); playSound('sfx-earn');
      }
  }
  else if(c.action === 'house-tax') {
      players.forEach(pl => { if(pl.isBankrupt) return; let tax = 0; for(let i in properties) { if(properties[i].owner === pl.id) { let h = properties[i].houses; if(h === 5) tax += c.hotel; else if(h > 0) tax += (h * c.house); } }
...
function processDebts() {
  ...
  if(debtor) {
      ...
      if(!debtAlertShown) {
          ...
          else if(amIActive) { openModal(`🚨 УВАГА: БОРГ!`, `<p>Ти пішов у мінус на <b style="color:#ef4444; font-size:18px;">i₴${Math.abs(debtor.money)}</b>.</p><p style="font-size:13px; color:#94a3b8;">Продай акції, заклади майно, візьми кредит або оголоси банкрутство (Здатися).</p>`, `<button class="btn-blue" onclick="closeModal()">Зрозуміло</button>`); debtAlertShown = true; }
      }
      return true; 
  }
  return false; 
...
function checkDebtResolution() {
    ...
    if(debtor && debtor.money >= 0) {
        ...
        if(!processDebts()) { updateUI(); if(isRolling) { nextTurn(); } broadcastState(); }
    }
}

function giveUpConfirm() { let p = players.find(x => x.debtMode) || players[turn]; openModal("🏳️ Здатися", `<p style="font-size:16px;"><b>${p.name}</b>, ти дійсно хочеш оголосити себе банкрутом і вийти з гри?</p>`, `<button class="btn-red" onclick="forceBankrupt()">Так, я банкрут</button><button class="btn-blue" onclick="closeModal()">Ні, я ще поборюсь</button>`); }
function forceBankrupt() { 
    ...
    for(let i in properties) { if(properties[i].owner === p.id) { delete properties[i]; document.getElementById(`houses-${i}`).innerHTML = ''; document.getElementById(`cell-${i}`).classList.remove('mortgaged'); document.getElementById(`cell-${i}`).style.borderColor = '#cbd5e1'; document.getElementById(`owner-${i}`).style.backgroundColor = 'transparent'; } }
    ...
    if (active.length === 1) { stopTimer(); openModal("🏆 ГРУ ЗАВЕРШЕНО!", `<h1 style="color:${active[0].color}; font-size:30px; text-shadow:0 0 15px ${active[0].color}88;">${active[0].name} ПЕРЕМІГ!</h1>`, `<button class="btn-green" onclick="window.location.reload()">Нова гра</button>`); broadcastState(); return; }
