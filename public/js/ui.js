...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. ГЕНЕРУЄМО ПОЛЯ ДЛЯ ГРАВЦІВ
    generatePlayerInputs();
    
    // 3. Налаштовуємо гучність
    updateVolume();
    
    // 4. Авто-логін — чекаємо підключення сокету
    function tryAutoLogin() {
        let saved = localStorage.getItem('poltavaUser');
        if (!saved) { updateProfileUI(); return; }
        let data = JSON.parse(saved);
        if (!socket) { updateProfileUI(); return; }
        if (!socket.connected) {
            // Чекаємо підключення
            socket.once('connect', () => {
                socket.emit('login', { nick: data.nick, pin: data.pin }, (res) => {
                    if (res && res.success) { currentUser = res.user; }
                    updateProfileUI();
                });
            });
        } else {
